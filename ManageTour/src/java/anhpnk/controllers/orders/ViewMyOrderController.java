 ...
 * @author Kim Anh
 */
public class ViewMyOrderController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<OrderDTO> listOrders = daoO.getOrdersByUsername(username);
            if(!listOrders.isEmpty()) {
                TourDAO daoT = new TourDAO();
                List<String> toursName = new ArrayList<>();
                String journey = null;
                for (OrderDTO order : listOrders) {
                    journey = daoT.getTourNameByTourID(order.getTourID());
                    toursName.add(journey);
                }
                request.setAttribute("TOUR_NAME", toursName);
                request.setAttribute("MY_ORDERS", listOrders);
            }
            
        } catch (Exception e) {
            log("Error at ViewMyOrderController: " + e.getMessage());
        } finally {
