...
public class ViewTourDetailsController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            TourDTO dto = dao.searchTourByID(tourID);
            request.setAttribute("TOURDETAILS", dto);
            
            String picLink = dto.getPicture();
            String[] splitLink = picLink.split(";");
            List<String> pictureSrc = new ArrayList<>();
            for (String link : splitLink) {
                if(!link.isEmpty()) {
                    String path = "images/" + tourID + "/" + link;
                    pictureSrc.add(path);
                }
                
            }
            request.setAttribute("LINKS", pictureSrc);
            
            PassengerDAO daoP = new PassengerDAO();
            int numOfPassenger = daoP.getNumOfPasseneger(tourID);
            int quantity = dto.getQuantity();
            HttpSession session = request.getSession();
            session.setAttribute("VALID_QUANTITY", quantity - numOfPassenger);
